...
_PIXABAY_EXTRACT_SYSTEM = (
    ...
    "그다음 남은 '업종/서비스'가 실제로 어떤 '장면·도구·작업 현장·결과물'로 사진에 찍히는지 구체적으로 떠올려서, "
    "픽사베이에서 그 업종과 직접 관련된 실사 사진이 나올 영문 검색어 3개를 콤마(,)로만 구분해서 출력해라.\n"
    "[엄수 규칙]\n"
    "- 반드시 '그 업종의 일하는 사람'이 등장하는 장면으로 만들어라. 각 검색어에 사람(worker/technician/caregiver/people 등)을 최대한 포함.\n"
    "- 추상적·은유적 단어(success, dream, future, journey) 절대 금지.\n"
    "- ★다음 '애매한 단어'를 단독으로 쓰면 엉뚱한 사진(돌·호박·동전·창고·발전소)이 나오니 절대 단독 사용 금지: "
    "stacked, storage, warehouse, pallet, shelf, box(es), facility, building, center, service, company, business, place, modern, interior. "
    "꼭 써야 하면 사람·행동과 결합해라(예: 'storage warehouse'(X) -> 'movers carrying boxes'(O), 'stacked boxes'(X) -> 'man moving boxes'(O)).\n"
    "- 설명/서론 없이 영문 구(句) 3개만.\n"
    "[예시]\n"
    "- '부산개인회생' -> 'lawyer meeting client, legal document signing, lawyer office desk'\n"
    "- '역삼동 헬스장' -> 'people working out gym, personal trainer, woman lifting dumbbell'\n"
    "- '요양원' -> 'caregiver helping elderly, nurse with senior patient, elderly care home'\n"
    "- '강남 이사' -> 'movers carrying furniture, moving truck loading, people moving boxes'\n"
    "- '보관이사' -> 'movers carrying boxes, moving truck loading, worker moving furniture'\n"
    "- '포장이사' -> 'movers packing boxes, mover wrapping furniture, moving truck workers'\n"
    "- '에어컨청소' -> 'technician cleaning air conditioner, hvac worker, repairman air conditioner'\n"
    "- '하수구막힘' -> 'plumber fixing drain, plumber working pipe, drain repair worker'\n"
    "- '강남 이사' -> 'moving truck, cardboard moving boxes, movers carrying furniture'\n"
    "- '보관이사' -> 'moving boxes stacked, storage warehouse, storage boxes'\n"
    "- '포장이사' -> 'packing moving boxes, moving truck, movers'\n"
    "- '에어컨청소' -> 'air conditioner cleaning, hvac technician, cleaning service'\n"
    "- '하수구막힘' -> 'plumber pipe, drain cleaning, plumbing repair'\n"
    "- '누수탐지' -> 'water leak pipe, plumber working, plumbing tools'\n"
    "- '인테리어' -> 'interior renovation, home remodeling, paint roller wall'\n"
    "- '철원 율무밭' -> 'farm field, grain harvest, agriculture'"
)
